...
import com.max2idea.android.limbo.virtualkeyboard.VirtualKeyboardController;
...
/**
 * 刷新された LimboSDLActivity
 * - Windows準拠 仮想ビルトインキーボード & マウスクイックバー（L/Rホールド、ホイール対応）
 * - バックグラウンド実行 & Picture in Picture (PiP) サポート
 */
public class LimboSDLActivity extends SDLActivity
    ...
    private View mVirtualKeyboardContainer;
    ...
    private void removeListeners() {
        ...
        if (mVirtualKeyboardController != null) {
            mVirtualKeyboardController.resetLatches();
        }
    ...
    private void setupWidgets() {
        ...
        mVirtualKeyboardContainer = findViewById(R.id.virtual_keyboard_container);
    ...
    /**
     * Windows準拠 仮想ビルトインキーボード & マウスクイックバーの初期化
     */
    private void setupVirtualKeyboard() {
        if (mVirtualKeyboardContainer == null) return;
        mVirtualKeyboardController = new VirtualKeyboardController(mVirtualKeyboardContainer, new VirtualKeyboardController.Listener() {
            @Override
            public void onVirtualKeyDown(int keyCode) {
                sendKeyEvent(null, keyCode, true);
            }

            @Override
            public void onVirtualKeyUp(int keyCode) {
                sendKeyEvent(null, keyCode, false);
            }
            ...
            @Override
            public void onVirtualMouseScroll(int deltaY) {
    ...
    private void sendMouseScroll(final int deltaY) {
        if (resettingLayout) return;
        mouseEventsExecutor.submit(new Runnable() {
            @Override
            public void run() {
                // ACTION_SCROLL (値: 8) を送信。y にスクロール量を渡す
                notifyAction(MachineAction.SEND_MOUSE_EVENT, new Object[]{0, MotionEvent.ACTION_SCROLL, 1, 0f, (float) deltaY});
            }
        });
    }
    ...
    private void toggleVirtualKeyboard() {
        if (mVirtualKeyboardContainer != null) {
            if (mVirtualKeyboardContainer.getVisibility() == View.VISIBLE) {
    ...
    protected void onUserLeaveHint() {
        ...
        // ホームボタンや他アプリへの移動時、バックグラウンド実行が有効であれば自動PiPへ遷移
