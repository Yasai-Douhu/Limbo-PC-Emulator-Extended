...
public class LimboSettingsManager extends PreferenceActivity {
    ...
    // バックグラウンド実行設定 (デフォルト: 有効)
    public static boolean getEnableBackgroundExecution(Context context) {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        return prefs.getBoolean("enableBackgroundExecution", true);
    }

    public static void setEnableBackgroundExecution(Context context, boolean enable) {
        SharedPreferences prefs = PreferenceManager.getDefaultSharedPreferences(context);
        SharedPreferences.Editor edit = prefs.edit();
        edit.putBoolean("enableBackgroundExecution", enable);
        edit.apply();
    }
